...
Full methodology, figures, and analysis are in the progress report.
...
The full pipeline is implemented in `beat-tracking-tcn.ipynb` and runs
end-to-end in Google Colab with a GPU runtime (tested on T4).
Datasets are not distributed with this repository; see "Data acquisition"
below for sourcing instructions.
...
1 of the progress report.

## Remaining work

Real-time inference pipeline, predictive scheduler for motor latency
compensation, and hardware integration with a mechanical actuator.
Scheduled for the final presentation one week from progress-report
submission.
